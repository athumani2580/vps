TANZANIA SCRIPT CREATED BY ALIEN
SLOWDNS OPENSSH

apt-get update && apt-get install -y sudo && sudo bash -c "$(curl -fsSL https://raw.githubusercontent.com/athumani2580/vps/main/slowdns/full.sh)"




HALOTEL ACTIVATE SAKURA ADMIN ONLY

sudo bash -c "$(curl -fsSL https://raw.githubusercontent.com/athumani2580/vps/main/Halotel.sh)"



SSHDROPBEAR SLOWDNS  ADMIN

sudo bash -c "$(curl -fsSL https://raw.githubusercontent.com/athumani2580/vps/main/slowdns/sshdropbear.sh)"



comfortable ssh script & stable

sudo bash -c "$(curl -fsSL https://raw.githubusercontent.com/athumani2580/vps/main/slowdns/stable2.sh)"


for test file 

sudo bash -c "$(curl -fsSL https://raw.githubusercontent.com/athumani2580/vps/main/test.sh)"

